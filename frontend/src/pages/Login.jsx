...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
      ...
      try {
        if (currentState === 'Sign Up') {   // we'll call the registration API
            const response = await axios.post(backendUrl + '/api/user/register' , {name,email,password})
            if(response.data.success){
              setToken(response.data.token)   // so that it'll store the token in state variable i.e. setToken
  ...
  }  // to prevent the page from re-load when we submit the form i.e. after we sign-in
